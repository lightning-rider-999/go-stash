...
[`docs/AGENTS.md`](docs/AGENTS.md). The generated per-command reference lives
under `docs/cli/`, starting at [`docs/cli/stash.md`](docs/cli/stash.md).
Deprecated commands are omitted from `docs/cli/` but remain invokable (with a
deprecation warning); `stash catalog` lists the complete surface, including
...
func main() {
	...
	// A generated operation. Pass c.GraphQL() as the client. The optional
	// FindFilterType fields are pointers; new(expr) (Go 1.26) takes their
	// addresses inline (Q *string, Per_page *int, …).
...
1. **`internal/gen`** drives the schema-agnostic
   ...
   (`cmd/stash/gen_commands.go`), and the machine-facing catalog.
