...
// TestReconcileKeepWaitingSignal pins the restructured reconcile contract: a job
// still in flight after a drop is the explicit (zero outcome, resolved=false,
// nil error) keep-waiting signal — never a bare nil,nil — and a terminal job is
// (outcome, resolved=true, nil).
func TestReconcileKeepWaitingSignal(t *testing.T) {
	t.Run("still running -> keep waiting", func(t *testing.T) {
		jt := &jobTracker{
			findJob:  fakeFindJob(findJobStep{snap: jobFixture(testJobID, stash.JobStatusRunning, "")}),
			progress: io.Discard,
		}
		outcome, resolved, err := jt.reconcile(context.Background(), testJobID)
		if err != nil {
			t.Fatalf("reconcile err = %v, want nil", err)
		}
		if resolved {
			t.Fatal("resolved = true for an in-flight job, want false (keep waiting)")
		}
		if outcome != (jobOutcome{}) {
			t.Fatalf("keep-waiting outcome = %+v, want the zero value", outcome)
		}
	})

	t.Run("terminal -> resolved", func(t *testing.T) {
		jt := &jobTracker{
			findJob:  fakeFindJob(findJobStep{snap: jobFixture(testJobID, stash.JobStatusFinished, "")}),
			progress: io.Discard,
		}
		...
		if !resolved {
			t.Fatal("resolved = false for a terminal job, want true")
		}
		if outcome.status != stash.JobStatusFinished {
			t.Fatalf("outcome status = %v, want FINISHED", outcome.status)
		}
	})
}

// TestWaitTimerStopped verifies the production timeout path stops its timer
// instead of leaking it: track is given a newTimer that records whether Stop was
// called, and a job that FINISHES before the timeout fires. On exit, Stop must
// have run.
func TestWaitTimerStopped(t *testing.T) {
	...
	jt := &jobTracker{
		...
		newTimer: func(time.Duration) (<-chan time.Time, func() bool) {
			return timerCh, func() bool { stopped.Store(true); return true }
		},
		progress: io.Discard,
		timeout:  30 * time.Second,
	}
	if err := jt.track(context.Background(), testJobID); err != nil {
		t.Fatalf("track returned %v, want nil (FINISHED before timeout)", err)
	}
	if !stopped.Load() {
		t.Fatal("track did not Stop the timeout timer on exit; it would leak until the deadline")
	}
}

// jobMutationSpec is a minimal job-returning mutation spec for runWait tests.
func jobMutationSpec() commandSpec {
	return commandSpec{
		Path:         []string{"scan"},
		OpName:       "MetadataScan",
		Query:        `mutation MetadataScan { metadataScan }`,
		Kind:         "mutation",
		ReturnType:   "ID",
		JobReturning: true,
	}
...
// TestRunWaitSigintDuringMutationExitsClean is the regression for the
// SIGINT-during-mutation consistency fix: a context cancelled before/while the
// enqueuing mutation runs must make runWait a clean stop (nil -> exit 0), the
// same disposition as a cancellation during the wait phase — not a transport
// failure (exit 4) just because the job had not been enqueued yet.
func TestRunWaitSigintDuringMutationExitsClean(t *testing.T) {
	// A server that blocks until the request's context is cancelled, modelling a
	// mutation in flight when SIGINT lands.
	...
	cmd.Flags().Duration(waitTimeoutFlag, 0, "")
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetContext(ctx)

	done := make(chan error, 1)
	go func() { done <- runWait(cmd, c, jobMutationSpec(), nil, "json") }()
	cancel()

	select {
	case err := <-done:
		if err != nil {
			t.Fatalf("runWait returned %v, want nil (SIGINT during mutation is a clean stop)", err)
		}
		if got := classifyExit(err); got != ExitOK {
			t.Fatalf("classifyExit = %v, want %v (exit 0)", got, ExitOK)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("runWait did not return within 5s of cancel")
	}
}
