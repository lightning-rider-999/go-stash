...
// history, so the STASHAPP_API_KEY environment variable is the preferred source
...
// before any client is built or any GraphQL request is sent, so a bad format is
// a usage error (exit 2) caught up front rather than after a write or an
// enqueued --wait job has already run.
func buildRootCommand() *cobra.Command {
	...
	// binary self-reports its real version/commit/date instead of "dev". A
	// release build (ldflags set version) is left untouched. Idempotent: once
	// version is no longer "dev", resolveBuildInfo is a no-op.
	...
	root := &cobra.Command{
		...
		RunE: helpOrUnknown,
		// PersistentPreRunE runs before the resolved command's RunE for every
		// command in the tree (cobra invokes only the nearest one, but every leaf
		// and group inherits the root's since none override it). It validates
		// --output up front so a bad format never reaches a client call.
	...
	// rather than recomputed inside the comparator (which runs O(n log n) times),
	// then paired back with the spec for the build loop.
...
// verbatim (not via SetFlagErrorFunc, which only covers flag parsing), so
// without this wrapper an extra positional argument would classify as the
// reserved ExitInternal instead of ExitUsage.
...
// before any side effect — a write mutation never runs, and a --wait job is never
// enqueued, on a bad format. cobra runs only the nearest PersistentPreRunE in
// the chain; no group or leaf overrides this one, so it fires for every command.
...
func newLeafCommandResolver(spec commandSpec, resolve clientResolver) *cobra.Command {
	leaf := &cobra.Command{
		...
		// the convenience flags. Rejecting extras (e.g. `scene list junk`) makes a
		// typo a usage error (exit 2) instead of a silently ignored argument.
		// cobra returns an Args error directly from Execute (it does not route it
		// through FlagErrorFunc), so it is wrapped here in newUsageError to land on
		// ExitUsage rather than the reserved ExitInternal.
		Args: usageNoArgs,
	}
	if spec.Deprecated {
	...
	leaf.RunE = func(cmd *cobra.Command, _ []string) error {
		...
		// importObjects takes a required file: Upload! that cannot be sent as JSON,
		// so it bypasses the generic dispatch for the hand-wired multipart path.
		if spec.OpName == importObjectsOpName {
			return runImportObjects(cmd, resolve, spec)
		}
