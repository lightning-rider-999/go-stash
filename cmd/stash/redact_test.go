			...
			"scenes": [
				{
					...
					"paths": {
						"stream": "https://stash.local/scene/42/stream?apikey=` + sampleJWT + `",
						"screenshot": "https://stash.local/scene/42/screenshot?apikey=` + sampleJWT + `&t=12"
					}
				}
			]
		}
...
func TestRedactAPIKeysScrubsJWT(t *testing.T) {
	...
	if !strings.Contains(s, "https://stash.local/scene/42/stream") {
...
func TestRedactPreservesOtherQueryParams(t *testing.T) {
	in := json.RawMessage(`["https://host/p?token=keepme&apikey=` + sampleJWT + `&page=3"]`)
	out, err := redactAPIKeys(in)
	if err != nil {
		t.Fatalf("redactAPIKeys: %v", err)
	}
