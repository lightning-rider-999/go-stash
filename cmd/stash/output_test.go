...
// TestUnwrapValuePropagatesDecodeError pins the err-3 fix: a malformed payload
// must surface the JSON decode error instead of collapsing to a silent nil.
func TestUnwrapValuePropagatesDecodeError(t *testing.T) {
	bad := json.RawMessage(`{"findScene":`) // truncated, not valid JSON
	v, err := unwrapValue(bad)
	if err == nil {
		t.Fatalf("unwrapValue(malformed) returned no error; got value %v", v)
	}
	if v != nil {
		t.Errorf("unwrapValue(malformed) value = %v, want nil alongside the error", v)
	}

	// The error-discarding shim kept for wait.go still tolerates the bad input by
	// returning nil, matching the job-id path's absent-id handling.
	if got := unwrapResult(bad); got != nil {
		t.Errorf("unwrapResult(malformed) = %v, want nil", got)
	}
}

// TestRenderersSurfaceDecodeError checks the ndjson and table renderers report a
// malformed payload as an error rather than emitting a garbled line. Redaction
// passes a non-JSON payload through untouched, so the decode happens in the
// renderer where the err-3 fix now propagates it.
func TestRenderersSurfaceDecodeError(t *testing.T) {
	bad := json.RawMessage(`{"findScene": tru`) // invalid JSON

	t.Run("ndjson", func(t *testing.T) {
		var buf bytes.Buffer
		if err := writeOutput(&buf, "ndjson", singleSpec, bad); err == nil {
			t.Fatalf("writeOutput ndjson(malformed) returned no error; wrote %q", buf.String())
		}
	})

	t.Run("table", func(t *testing.T) {
		var buf bytes.Buffer
		if err := writeOutput(&buf, "table", singleSpec, bad); err == nil {
			t.Fatalf("writeOutput table(malformed) returned no error; wrote %q", buf.String())
		}
	})
}

// TestUnknownFormatClassifiesAsUsage pins the cli sub-3 fix: an unrecognised
// --output value is a usage error (exit 2), not an internal failure (exit 1).
func TestUnknownFormatClassifiesAsUsage(t *testing.T) {
	err := writeOutput(&bytes.Buffer{}, "xml", singleSpec, json.RawMessage(`{}`))
	if err == nil {
		t.Fatal("expected an error for an unknown format")
	}
	if got := classifyExit(err); got != ExitUsage {
		t.Errorf("classifyExit(unknown format) = %+v, want ExitUsage (%+v)", got, ExitUsage)
	}
}

// TestIsValidOutputFormat exercises the helper commands.go's PreRunE reads to
// pre-validate --output against the same set writeOutput renders.
func TestIsValidOutputFormat(t *testing.T) {
	for _, f := range []string{"", "json", "ndjson", "table", "yaml"} {
		if !isValidOutputFormat(f) {
			t.Errorf("isValidOutputFormat(%q) = false, want true", f)
		}
	}
	for _, f := range []string{"xml", "csv", "JSON", "yml"} {
		if isValidOutputFormat(f) {
