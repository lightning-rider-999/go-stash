...
//
// The subscription gets its own cancellable context, cancelled on every return
// path from streamEvents — a clean ctx-cancel, a closed stream, or a write
// error. [stash.Subscribe] ties its dialler/reconnect goroutine to this context,
// so a streamEvents return from a downstream write failure (where the parent ctx
// is still live) still tears the socket down deterministically rather than
// leaking the goroutine until the process exits.
...
// classifyExit routes it to the transport exit code. An error that already
// carries a [*stash.TransportError] in its chain classifies as-is; any other
// terminal cause (reconnect exhaustion) is wrapped in the CLI transportError so
// the same code results. Binding the matched value with errors.AsType lets the
// classified error be returned directly rather than re-walking the chain in
// classifyExit.
func classifyStreamErr(err error) error {
	if te, ok := errors.AsType[*stash.TransportError](err); ok {
		return te
	}
