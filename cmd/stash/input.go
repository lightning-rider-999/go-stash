...
func applyIDFlag(cmd *cobra.Command, args map[string]bool, vars map[string]json.RawMessage) error {
	...
	case args["ids"]:
		...
		// encoded is the []byte from json.Marshal, which is assignable to
		// json.RawMessage in the slice literal, so the explicit conversion is
		// redundant.
		list, err := json.Marshal([]json.RawMessage{encoded})
		if err != nil {
	...
	case args["id"]:
		...
		vars["id"] = encoded
	default:
		// Offering --id where the operation accepts neither ids nor id is a
		// caller mistake: a usage error (exit 2).
		return newUsageError(errors.New("--id is not accepted by this operation"))
	}
	return nil
}
...
func applyFilterFlags(cmd *cobra.Command, args map[string]bool, vars map[string]json.RawMessage) error {
	...
	if raw, ok := vars["filter"]; ok {
		if err := json.Unmarshal(raw, &filter); err != nil {
			return newUsageError(fmt.Errorf("--input filter must be a JSON object: %w", err))
		}
	}
	...
	for _, f := range convenienceFilterFlags {
		...
		if len(f.enumValues) > 0 && !slices.Contains(f.enumValues, val) {
			return newUsageError(fmt.Errorf("--%s must be one of %v, got %q", f.flag, f.enumValues, val))
		}
...
func encodeFilterValue(field, val string) (json.RawMessage, error) {
	if slices.Contains(numericFilterFields, field) {
		...
		if !json.Valid([]byte(val)) {
			return nil, newUsageError(fmt.Errorf("--%s must be an integer, got %q", filterFlagFor(field), val))
		}
		var n int
