...
func TestCleanupSubscriptionBacksStopWhenCloseFails(t *testing.T) {
	...
	// closeErr set means genqlient's Close returns early without closing errChan,
	// exactly the dead-socket path. The errChan drainer therefore can never be
	// released by errChan closing; it must terminate on cleanup's own deadline, or
	// it leaks for the life of the process (Finding sub-1).
	wsc := &fakeWSClient{closeErr: errors.New("failed to send closure message"), errChan: make(chan error)}

	// Sample the baseline after settling so the only post-cleanup delta is a leak.
	settle()
	before := runtime.NumGoroutine()

	cleanupSubscription[bool](wsc, sc, wsc.errChan, nil)

	// cleanupSubscription returns only after wg.Wait, so its drainers and deadline
	// watcher have already exited by here. Poll back to baseline to prove the
	// errChan drainer did not outlive the unclosed errChan.
	deadline := time.Now().Add(2 * time.Second)
	for {
		settle()
		if runtime.NumGoroutine() <= before {
			break // drainers released and exited; no leak
		}
		if time.Now().After(deadline) {
			t.Fatalf("goroutine count stayed at %d (baseline %d): a cleanup drainer leaked past the unclosed errChan",
...
// --- Finding sub-2: cleanup drains the data stream so a forwardDataFunc send
// parked on the unbuffered per-subscription channel can complete, releasing the
// read goroutine instead of leaking it.

func TestCleanupSubscriptionDrainsInFlightDataSend(t *testing.T) {
	fc := newFakeConn()
	sc := newSerialConn(fc, time.Hour, nil)

	// errChan closes cleanly here; the focus is the stream drain, not the errChan.
	wsc := &fakeWSClient{errChan: make(chan error)}

	// Model genqlient's read goroutine: parked on a blocking send into the
	// unbuffered channel that ultimately feeds stream, exactly as forwardDataFunc
	// is when ctx-cancel races a pending event. Nothing else reads stream once
	// runSubscription's loop has returned, so without a drain this send — and the
	// goroutine holding it — leaks forever.
	stream := make(chan bool) // unbuffered, like the generated data channel
	parked := make(chan struct{})
	readDone := make(chan struct{})
	go func() {
		defer close(readDone)
		close(parked)
		stream <- true // blocks until cleanup drains stream
	}()

	<-parked
	// Let the send actually park on the channel before cleanup runs.
	time.Sleep(20 * time.Millisecond)

	settle()
	before := runtime.NumGoroutine()

	cleanupSubscription[bool](wsc, sc, wsc.errChan, stream)

	// The parked send must have been received by the drainer, releasing the read
	// goroutine.
	select {
	case <-readDone:
	case <-time.After(2 * time.Second):
		t.Fatal("the in-flight data send was never drained: the read goroutine leaked")
	}

	// And no cleanup goroutine outlived the call.
	deadline := time.Now().Add(2 * time.Second)
	for {
		settle()
		if runtime.NumGoroutine() <= before {
		...
		if time.Now().After(deadline) {
			t.Fatalf("goroutine count stayed at %d (baseline %d): a cleanup drainer leaked",
				runtime.NumGoroutine(), before)
		}
	}
}

// --- Finding sub-3 (subs): serialConn.Close is safe under concurrent callers;
// close(stop) must not double-close and panic.

func TestSerialConnCloseConcurrentSafe(t *testing.T) {
	fc := newFakeConn()
	sc := newSerialConn(fc, time.Hour, nil)
	...
	for range callers {
		go func() {
			defer wg.Done()
			<-start // line them up so the close(stop) genuinely races
			if err := sc.Close(); err != nil {
	...
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("concurrent Close calls did not all return (a double-close panic or a wedge)")
	}
...
// --- Finding (keep WithKeepaliveInterval live): the option sets the configured
// interval on subOptions, and the public Subscribe path threads it to the dialer.

func TestWithKeepaliveIntervalSetsInterval(t *testing.T) {
	const want = 7 * time.Second
	o := defaultSubOptions()
	if o.interval == want {
		t.Fatalf("default interval already %v; choose a distinct test value", want)
	}
	WithKeepaliveInterval(want)(&o)
	if o.interval != want {
		t.Errorf("interval after WithKeepaliveInterval(%v) = %v, want %v", want, o.interval, want)
	}

	// A non-positive interval is honoured verbatim (it disables keepalive).
	WithKeepaliveInterval(0)(&o)
	if o.interval != 0 {
		t.Errorf("interval after WithKeepaliveInterval(0) = %v, want 0", o.interval)
	}
}

// --- Finding sub-5: the exponential backoff cannot overflow base << shift and
// wrap to a small or negative duration for a large attempt count.

func TestBackoffDelayNeverOverflows(t *testing.T) {
	const base = 500 * time.Millisecond
	const maxDelay = 30 * time.Second

	// Early attempts grow as expected and stay within the cap.
	if got := backoffDelay(base, maxDelay, 1); got != base {
		t.Errorf("attempt 1 delay = %v, want %v", got, base)
	}
	if got := backoffDelay(base, maxDelay, 2); got != 2*base {
		t.Errorf("attempt 2 delay = %v, want %v", got, 2*base)
	}

	// Every attempt, including absurdly large ones that would overflow a naive
	// base<<(attempt-1), is clamped to maxDelay and never goes non-positive.
	for _, attempt := range []int{8, 63, 64, 65, 100, 1_000_000, 1 << 30} {
		got := backoffDelay(base, maxDelay, attempt)
		if got <= 0 {
			t.Errorf("attempt %d delay = %v, want a positive, clamped duration", attempt, got)
		}
		if got > maxDelay {
			t.Errorf("attempt %d delay = %v, exceeds cap %v", attempt, got, maxDelay)
		}
	}

	// The clamp is reached and held: a large attempt yields exactly maxDelay.
	if got := backoffDelay(base, maxDelay, 1000); got != maxDelay {
		t.Errorf("attempt 1000 delay = %v, want clamp %v", got, maxDelay)
	}
}
