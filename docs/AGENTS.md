...
`stash catalog` is the authoritative listing of the **complete** operation
surface, including deprecated operations. Deprecated commands are omitted from the
generated per-command reference under `docs/cli/`, but they stay invokable (they
print a deprecation warning to stderr), so the catalog — not `docs/cli/` — is the
exhaustive index of what the CLI can do.
