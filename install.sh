...
# `stash` binary, and installs it into a directory on (or addable to) your PATH.
#
...
main() {
	# Self-test hook: when STASH_INSTALL_SELFTEST is set, a test harness has
	# sourced this file only to exercise the helper functions in isolation, so
	# main() returns immediately and installs nothing. In normal use the variable
	# is unset and the full install runs. Keeping this check INSIDE main() lets
	# the file end with an unconditional `main "$@"` on its last line (see below).
	if [ -n "${STASH_INSTALL_SELFTEST:-}" ]; then
		return 0
	fi
	...
	# stash_<version>_<os>_<arch>.tar.gz — by suffix, not reconstruction.
	...
	tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t stash-install)"
	...
	# --version because cmd/stash sets the root command's Version field. Both the
	# binary's stdout and stderr are routed to our stderr so a broken binary's
...
# so nothing is installed. The STASH_INSTALL_SELFTEST guard now lives inside
