// Package redact scrubs the instance API key out of decoded JSON payloads.
//
// Stash signs media URLs — Scene.paths.stream and friends — with the instance
// API key as an `apikey` query parameter
// (http://host/scene/42/stream?apikey=<JWT>). That JWT travels in the default
// scene payload, so callers run a payload through [APIKeys] before it is
// printed or logged.
package redact
...
// APIKeys walks the decoded JSON value recursively and scrubs the API key out
// of every pre-signed media URL it carries.
//
...
// sibling parameters are preserved in their decoded form. Strings that are not
// URLs, or URLs without an apikey, are returned untouched. The value is
// re-encoded; on any decode failure the original bytes pass through unchanged
// so redaction can never corrupt a payload.
