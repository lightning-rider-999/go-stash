...
// matters, is reachable through a sibling field; e.g. VideoFile.fingerprints
// covers the parameterized VideoFile.fingerprint accessor).
...
// is neither a referenceable entity nor a flat value type. SceneGroup,
// SceneMovie, GroupDescription, and SceneMarkerTag are the SDL instances.
...
func (fs *FragmentSet) Names() []string {
	names := make([]string, 0, len(fs.bodies))
	for name := range fs.bodies {
		names = append(names, name)
	}
	sort.Strings(names)
	return names
...
func (fs *FragmentSet) PathNamedTypes() []string {
	names := make([]string, 0, len(fs.path.types))
	for name := range fs.path.types {
		names = append(names, name)
	}
	sort.Strings(names)
	return names
