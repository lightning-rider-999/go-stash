...
type Operation struct {
	Name  string        // exported operation name (FindScenes, SceneCreate, ...)
	Op    ast.Operation // query | mutation | subscription
	Field string        // root field name (findScenes, sceneCreate, ...)
...
// verbatim from the SDL with deprecated arguments dropped (A6, e.g. findScenes
// keeps ids:[ID!] and omits scene_ids:[Int!]); interface/union selections carry
// __typename (A5).
func BuildOperations(s *ast.Schema, fs *FragmentSet) ([]Operation, error) {
	...
	for _, ot := range []ast.Operation{ast.Query, ast.Mutation, ast.Subscription} {
		sorted := append(ast.FieldList(nil), RootFields(s, ot)...)
